...
checking is function of both the distance of the previous check $i_{C_{\text{last}}$ and the gage 
...
\begin{equation}
  C_{i} = \frac{\beta.\exp(p(c)_{i})}{\beta.\exp(p(c)_{i}) + \beta.\exp(p(\bar{c})_{i})}
\end{equation}

\section{Fit}

In oder to perform a fitting procedure, we will need 3 things:
\begin{itemize}
  \item{the vector of right and wrong answer $y$,}
  \item{the trial in which the monkey performed a check,}
  \item{the gauge filling rule (so that we can give the model $g_{\text{last}}$ at any trial).}
\end{itemize}

Note thus that these are \textit{meta} models since they can only predict the 
checking behavior. In other words, they need to know the type I information (outcomes in the main task) to perform 
their predictions.

By computing a BIC for each model, one can get the best model: the one with (i) the few number of free parameter(s)
